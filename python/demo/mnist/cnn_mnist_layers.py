...
def cnn_model_fn(features, labels, mode):
    ...
    input_layer = tf.reshape(features["x"], [-1, 28, 28, 1])  # batch_size 每次训练使用的训练集的子集的大小
    ...
    conv1 = tf.layers.conv2d(  # 使用conv2d来创建卷积层, 使用了32个5×5的filter，并且使用ReLU激活函数
        inputs=input_layer,
        filters=32,
        kernel_size=[5, 5],
    ...
    # Pooling Layer #1 使用max_pooling2d来创建pooling层, 需要2×2的filter做max pooling，步长为2
    ...
    # Output Tensor Shape: [batch_size, 7 * 7 * 64]
    pool2_flat = tf.reshape(pool2, [-1, 7 * 7 * 64])  # 把pool2的dimension从4变成2

    # Dense Layer
    # Densely connected layer with 1024 neurons 神经元
    # Input Tensor Shape: [batch_size, 7 * 7 * 64]
    ...
    logits = tf.layers.dense(inputs=dropout, units=10)  # 激活函数使用的是默认值（线性激活函数）
